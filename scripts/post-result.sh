...
#                                                                                   > push-failed.md
#   post-result.sh issue_conflict      <run_url> <target_branch> <conflict_subs>    > conflict-comment.md
#   post-result.sh issue_mr_multi      <mr_list_file> <target_branch>               > issue-mr-comment.md  (one line per MR: 'sub|url')
...
#
# Optional env:
#   POST_MERGE_NOTICE   one-line text appended to issue_pushed_multi /
#                       issue_mr_multi (e.g. deploy timing). Empty / unset =
#                       no notice line.
...
case "$MODE" in
  ...
  issue_conflict)
    # Args: <run_url> <target_branch> <conflict_subs_str>
    # Used by direct_push mode when precheck-remote-base.sh detects the remote
    # advanced during the run. The body explicitly tells the operator the next
    # step so a stalled issue doesn't sit there waiting for someone to guess.
    RUN_URL="$2"
    TARGET_BRANCH="$3"
    CONFLICT_SUBS="$4"
    printf '**Push aborted: `%s` advanced during the run.**\n\n' "$TARGET_BRANCH"
    printf 'Affected subprojects: **%s**\n\n' "$CONFLICT_SUBS"
    printf 'Someone — human or another Claude run — pushed commits to `%s` between our `git reset --hard` and the moment we tried to push. We refuse to rebase Claude'"'"'s commits onto unknown work, so no code was published.\n\n' "$TARGET_BRANCH"
    printf '**Next steps:**\n'
    printf -- '- Reply with `@claude` to retry against the now-current `%s`.\n' "$TARGET_BRANCH"
    printf -- '- Or pull the run log, replay the change manually, and resolve any conflicts yourself.\n\n'
    printf '[Run log](%s)\n' "$RUN_URL"
    ;;

  issue_mr_multi)
    # Args: <mr_list_file> <target_branch>
    # List file: one 'sub|mr_url' line per MR opened.
    LIST_FILE="$2"
    TARGET_BRANCH="${3:-dev}"
    if [ ! -f "$LIST_FILE" ]; then
    ...
    COUNT=$(grep -c '|' "$LIST_FILE" || true)
    if [ "$COUNT" = "1" ]; then
      printf '**Claude opened 1 MR on GitLab against `%s`:**\n\n' "$TARGET_BRANCH"
    else
      printf '**Claude opened %s MRs on GitLab against `%s`:**\n\n' "$COUNT" "$TARGET_BRANCH"
    fi
  ...
  mr_body)
    # Args: <repo_full> <issue_num> <run_url>
    # Used as the GitLab MR description. Keep it short — Summary table is the
    # main payload; everything else is reproducibility metadata.
    REPO_FULL="$2"
    ISSUE_NUM="$3"
    RUN_URL="$4"
    printf 'Source issue: %s#%s\n\n' "$REPO_FULL" "$ISSUE_NUM"
    render_section 50
    printf '\n\n---\nAuto-generated by Claude. Run: %s\n' "$RUN_URL"
    ;;
